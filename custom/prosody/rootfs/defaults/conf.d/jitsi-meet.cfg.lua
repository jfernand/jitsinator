...
admins = {
    ...
    "{{ $JIGASI_XMPP_USER }}@{{ $XMPP_AUTH_DOMAIN }}", -- TODO stop generating jigasi password
    {{ end }}
...
unlimited_jids = { -- jfs TODO possibly not needed
    "focus@{{ $XMPP_AUTH_DOMAIN }}",
    "{{ $JVB_AUTH_USER }}@{{ $XMPP_AUTH_DOMAIN }}"
}
...
muc_mapper_domain_prefix = "{{ $XMPP_MUC_DOMAIN_PREFIX }}"; -- TODO not in prod

recorder_prefixes = { "{{ $JIBRI_RECORDER_USER }}@{{ $XMPP_HIDDEN_DOMAIN }}" }; -- TODO not in prod

http_default_host = "{{ $XMPP_DOMAIN }}" -- TODO not in prod
...
-- TODO wait to see what the above generates
    modules_enabled = {
        "bosh";
        "features_identity";
        {{ if $ENABLE_XMPP_WEBSOCKET }}
        "websocket";
        "smacks"; -- XEP-0198: Stream Managementtt
        {{ end }}
        "conference_duration";
        {{ if $ENABLE_LOBBY }}
        "muc_lobby_rooms";
